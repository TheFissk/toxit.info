...
class Inference_task(models.Model):
    ...
    start_sched = models.DateTimeField(
        help_text='Requested start time')
    ...
    def get_unique_edge_pairs(self) -> tuple:
        ...
        In the form:(From_Sub, To_Sub, Strength)
        """
        modPairs = []
        authorPairs = []
        subreddit_list = self.get_subreddits_for_inference_task()
        for sub in subreddit_list:
            from_Sub = sub.subreddit.display_name
            edges = json.loads(sub.edges)
            for to_Sub in edges['mods'].keys():
                modPairs.append((from_Sub, to_Sub, edges['mods'][to_Sub]))

            for to_Sub in edges['authors'].keys():
                authorPairs.append(
                    (from_Sub, to_Sub, edges['authors'][to_Sub]))
            # no need to check for uniqueness, the property is guarenteed
        return modPairs, authorPairs
...
class Comment_result(models.Model):
    ...
    permalink = models.TextField(
        help_text='The permalink to the comment sample')

    mhs_score = models.FloatField(default=0,
                                  help_text='The mhs inference score of the sample')

    comment_body = models.TextField(
        help_text='The comment sample')

    username = models.CharField(max_length=32,
                                help_text='The username of the commentor')
